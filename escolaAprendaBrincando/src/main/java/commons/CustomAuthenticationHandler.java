	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		for (GrantedAuthority ga : permissoes) {
			if (ga.getAuthority().equals("ROLE_ADMINISTRADOR")) {
				redirectStrategy.sendRedirect(request, response, "/admin/menuadministrativo.xhtml");
			} else if (ga.getAuthority().equals("ROLE_ALUNO")) {
				redirectStrategy.sendRedirect(request, response, "/aluno/menualuno.xhtml");
			} else if (ga.getAuthority().equals("ROLE_PROFESSOR")) {
				redirectStrategy.sendRedirect(request, response, "/professor/menuprofessor.xhtml");
			}
		}
